    ...
    if (handle)
    {
        ...
        while (true)
        {
            ...
            while (getline(sData, line)) {
                ...
                tempvector.push_back(stod(adjprice));
            }
            itr->second.Set_AdjClose(tempvector);
            if(itr->second.Get_AdjClose().size() != 61)
            {
                cout << "NO ENOUGH DATA!! WRONG!!" << endl;
                cout << itr->first << " " << itr->second.Get_AdjClose().size() << endl;
            }
            itr++;
            count++;
            if (count % 50 == 0)
                cout << "- - - - - downloading " << count/5 << "% - - - - -" << endl;
                
            if (itr == StockList.end())
            {
                cout << "- - - - - downloading 100% - - - - -" << endl;
                break;
            }
        }
        free(data.memory);
...
int ExtractSPY(vector<double>& SPY, vector<string>& DATE, string StartDate, string EndDate)
{
